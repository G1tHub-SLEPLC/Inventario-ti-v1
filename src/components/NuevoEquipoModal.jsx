...
export default function NuevoEquipoModal({ isOpen, onClose }) {
  ...
  const [isOcVerifiedMulti, setIsOcVerifiedMulti] = useState(false);
  ...
  const handleFileChange = async (type, file) => {
    ...
    if (type !== 'factura') {
      const { Marca, Modelo } = formData;
      if (!Marca || !Modelo || Marca.trim() === '' || Modelo.trim() === '') {
        showToast(
          'Faltan Datos',
          'Debe ingresar la Marca y el Modelo antes de subir la Orden de Compra para realizar las validaciones de cantidades correctamente.',
          'warning'
        );
        return;
      }
    ...
    const matchedCode = extractedCode && existingCodes.some(code => code.toLowerCase() === extractedCode.toLowerCase())
      ? extractedCode
      : existingCodes.find(code => fileNameLower.includes(code.toLowerCase()));
    ...
    if (type === 'factura') {
      if (matchedCode && (!formData[fieldName] || formData[fieldName].trim() === '')) {
        ...
        setFacturaFile(file);
        if (extractedCode && (!formData[fieldName] || formData[fieldName].trim() === '')) {
    ...
    // Flujo para Orden de Compra (PDF Parsing)
    if (file.type === 'application/pdf' || fileNameLower.endsWith('.pdf')) {
      try {
        ...
        const codeToUse = extractedCode || pdfData.nombreOC || cleanName;
        let newData = { ...formData, [fieldName]: codeToUse };

        if (pdfData.isConvenioMarco && pdfData.convenioMarco && pdfData.tipoPublicacion === 'Convenio Marco') {
          newData['ID Publicación'] = pdfData.convenioMarco;
        }

        if (pdfData.tipoPublicacion) {
          newData['Tipo Publicación'] = pdfData.tipoPublicacion;

          if (pdfData.tipoPublicacion === 'Compra Ágil') {
            await showAlertConfirm(
              'ID de Publicación Faltante',
              'Se ha detectado que esta orden corresponde a una <b>Compra Ágil</b>.<br/><br/>Por favor, busca el ID de la compra agil e ingrésalo manualmente en el campo <b>ID Publicación</b>.'
            );
        ...
        if (pdfData.proveedor) {
          newData['Proveedor'] = pdfData.proveedor;
        }

        if (pdfData.cantidad > 0) {
          const sameProductEqs = equipos.filter(eq =>
            eq['Orden de Compra'] === codeToUse &&
            eq['Marca'] === formData['Marca'] &&
            eq['Modelo'] === formData['Modelo']
          );
          const registeredCount = sameProductEqs.length;

          if (registeredCount >= pdfData.cantidad) {
            showToast('Límite Alcanzado', `Ya se han ingresado los ${pdfData.cantidad} equipos de esta marca/modelo según la Orden de Compra. Se cancelará la subida para evitar duplicados.`, 'error');
            return;
          } else if (registeredCount > 0) {
            const faltantes = pdfData.cantidad - registeredCount;
            const confirm = await showAlertConfirm('Ingreso Parcial Detectado', `Ya se han ingresado ${registeredCount} de ${pdfData.cantidad} equipos de esta marca/modelo de esta OC. ¿Desea ingresar los ${faltantes} restantes?`);
            if (!confirm) {
              return;
            }
            setIsMultiMode(true);
            setIsOcVerifiedMulti(true);
            newData.multiCount = faltantes;
          } else {
            newData.multiCount = pdfData.cantidad;
        ...
        setFormData(newData);
        if (pdfData.cantidad > 0) {
          showToast('Lectura Exitosa', `Se ha leído la Orden de Compra. Cantidad detectada: ${pdfData.cantidad}.`, 'success');
        }
      } catch (err) {
        console.error('Error al parsear PDF:', err);
        showToast('Atención', 'No se pudo leer la cantidad automáticamente del PDF. Deberás ingresar los números de serie manualmente.', 'warning');
        setOcFile(file);
        if (extractedCode && (!formData[fieldName] || formData[fieldName].trim() === '')) {
          setFormData(prev => ({ ...prev, [fieldName]: extractedCode }));
        }
  ...
  const handleSubmit = async (e) => {
    ...
    // VALIDACIÓN: Descripción del Bien
    if (!formData['Descripción del Bien'] || formData['Descripción del Bien'].trim() === '') {
      showToast(
        'Descripción Requerida',
        'Debe seleccionar o ingresar una Descripción del Bien para guardar el equipo.',
        'error'
      );
      return;
    }
    ...
    if (isMultiMode) {
      ...
      let uniqueSerials = [...new Set(parsedSerials)];

      if (uniqueSerials.length === 0) {
        const countToGenerate = formData.multiCount || 0;
        if (countToGenerate > 0 && isOcVerifiedMulti) {
          for (let i = 0; i < countToGenerate; i++) {
            uniqueSerials.push(`FALTA-${Date.now()}-${Math.floor(Math.random() * 10000) + i}`);
          }
        } else {
          showToast('Atención', 'No se detectó la cantidad en la Orden de Compra o la verificación automática no está activa. Debes ingresar los números de serie manualmente.', 'error');
          return;
        }
      }
      ...
      if (facturaFile && !facturaHasFile) {
        try {
          const facturaCode = formData['Factura'];
          const storageKeyF = (facturaCode && facturaCode.trim() !== '—' && facturaCode.trim() !== '')
            ? `factura_${facturaCode.trim().toLowerCase()}`
            : `factura_${firstId}`;
          await saveDocument(storageKeyF, 'factura', facturaFile);
          factSaved = true;
        } catch (err) {
          console.error('Error saving multiple invoice:', err);
        }
      }

      if (ocFile && !ocHasFile) {
        try {
          const ocCode = formData['Orden de Compra'];
          const storageKeyO = (ocCode && ocCode.trim() !== '—' && ocCode.trim() !== '')
            ? `oc_${ocCode.trim().toLowerCase()}`
            : `oc_${firstId}`;
          await saveDocument(storageKeyO, 'oc', ocFile);
          ocSaved = true;
        } catch (err) {
          console.error('Error saving multiple PO:', err);
        }
      }
      ...
      if (cascadeEquipos.length > 0) {
        const resUpdate = await updateEquiposMasivo(cascadeEquipos);
      ...
      if (success && nuevosEquipos.length > 0) {
        const resAdd = await addMasivo(nuevosEquipos);
        if (resAdd && resAdd.success === false) {
          success = false;
    ...
    let serial = formData['Nº de serie'] ? String(formData['Nº de serie']).trim() : '';
    if (!serial && formData.multiCount === 1) {
      serial = `S/N-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
      formData['Nº de serie'] = serial;
    }
        ...
        <div className="overflow-y-auto pr-2 custom-scrollbar flex-1 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {isMultiMode && (
                <div className="flex items-center gap-2 bg-yellow-50 px-3 py-1.5 rounded-lg border border-yellow-200 shadow-sm animate-fade-in">
                  <span className="text-[10px] font-bold text-yellow-800 uppercase">Cantidad Total:</span>
                  <input
                    type="number"
                    min="1"
                    max="5000"
                    value={formData.multiCount || ''}
                    onChange={(e) => setFormData(prev => ({ ...prev, multiCount: parseInt(e.target.value) || 0 }))}
                    className="w-16 rounded bg-white border-yellow-300 shadow-sm border p-1 text-xs text-center font-bold text-gray-800 focus:border-yellow-500 focus:ring-yellow-500"
                    placeholder="Ej. 10"
                  />
                </div>
              )}
                  ...
                  {isMultiMode ? (
                    <div className="space-y-2">
                      <textarea
                        ...
                        placeholder={isOcVerifiedMulti ? "Pegue aquí los números de serie o deje en blanco para auto-generar los restantes." : "Pegue aquí los números de serie (uno por línea o separados por coma)"}
                        required={!isOcVerifiedMulti}
                      />
